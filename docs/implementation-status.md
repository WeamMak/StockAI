...
- Active task: T04 is implemented and verified locally on its feature branch;
  branch review, dev validation, and promotion remain pending.
...
| T04 | Implemented locally | Red-green tests cover strict environment-bound candidate inputs, bounded typed fictional output, output-schema rejection, environment mismatch, opaque bearer verification, missing/wrong credentials, safe timeout and unavailable-source retries, non-echoing errors, structured logs, and bounded Prometheus call/duration/failure/timeout/retry metrics. `make check` passed with 155 unit tests, strict mypy over 48 files, architecture and lock checks, and 91% branch-aware coverage. `make test-integration` passed 3 tests using the Python MCP client against an actual authenticated Uvicorn Streamable HTTP server; the client discovered and invoked the tool and scraped its resulting metric. | The test adapter is deterministic and fictional. Real Odoo JSON-2 contract verification and the production adapter remain T10–T11; LangGraph/API use of this MCP tool remains T05; container and deployment configuration remain later approved tasks. Feature-branch review and the required dev/prod promotion workflow are still pending. |
