variable "aws_account_id" {
  description = "Twelve-digit AWS account ID that owns the prod resources"
  type        = string
...
variable "aws_region" {
  description = "Approved AWS region for the StockAI prod environment"
  type        = string
  default     = "us-east-1"
...
variable "data_volume_size_gib" {
  description = "Initial GiB size of each retained prod data volume"
  type        = number
  default     = 5
...
variable "domain_name" {
  description = "Existing user-owned DNS domain used for the prod callback URL"
  type        = string
...
variable "enable_cognito_deletion_protection" {
  description = "Protect the production Cognito user pool from deletion"
  type        = bool
  default     = true
}

variable "enable_odoo_key_bootstrap" {
  description = "Temporarily attach exact-secret write permission for the finite prod Odoo key job"
  type        = bool
  default     = false
}
...
variable "owner_name" {
  description = "Owner recorded on prod AWS resources"
  type        = string
  default     = "weam"
}

variable "worker_availability_zone" {
  description = "T16 Availability Zone dedicated to the prod worker ASG"
  type        = string
}

variable "worker_role_name" {
  description = "T16 IAM role name used only by prod workers"
  type        = string
}
