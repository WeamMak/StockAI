  ...
  return (
    <section aria-labelledby={`preferences-${preferences.profile_id}`}>
      ...
      <dl className="evidence-grid">
        ...
        <div>
          <dt>Price premium</dt>
          <dd>{preferences.max_price_premium_percent}% ({preferences.enforcement_mode})</dd>
        </div>
        <div><dt>Cheapest eligible cost</dt><dd>{preferences.cheapest_eligible_cost}</dd></div>
      </dl>
      <ul className="tag-list" aria-label="Offer premium results">
        {preferences.offer_results.map((result) => (
          <li key={result.offer_id}>
            {result.offer_id}: {result.premium_percent}% — {label(result.outcome)}
          </li>
        ))}
      </ul>
    </section>
